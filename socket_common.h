 ...
 * socket_common.h
 *
 * Common socket routines used by walnut and walnutd
 *
 ...
 */
static inline int walnut_socket(char *path)
{
...
/*
 * Look at the registered list of clients and ask walnutd for each of them if the
 * client is the active_walnut - return the client if that is the case.
 *
 * TODO: This suggests that the data in socket_client could go in the walnut_node
...
active_socket_client(struct client_list *list)
{
	...
	struct walnut_node *node;

	pthread_mutex_lock(&list->mutex);

	client = list->head;
	while (client != NULL) {
		node = search_walnuts_path(client->root_path);
		if (node == active_walnut)
			break;
		client = client->next;
	}
...
/*
 * Map a socket client (walnut instance) to a client ID 
 */
static inline int socket_client_id(struct socket_client *client)
{
	...
	walnut = search_walnuts_path(client->root_path);
	if (walnut == NULL)
		return -1;
	return walnut->id;
}
...
/*
 * Obtains the walnut ID from its configuration file
 *
 * This lookup is what effectively chooses a walnut into which an incoming call
 * or SMS, etc. will be placed. The digit must correspond to the digit
 * appended to the caller ID from the calling service
 */
static inline struct socket_client *
socket_client_from_id(int walnutID, struct client_list *list)
{
	struct socket_client *client;
	char walnutID_str[0];
