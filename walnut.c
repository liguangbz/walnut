...
static int do_command(struct walnut_cmd_arg *cmd);

/*
 * Open connection with walnutd and return file descriptor.
 */
static int open_conn(void)
{
	...
	if (sd == -1) {
		perror("Failed to connect to walnutd");
		return -1;
	}
	...
	if (connect(sd, (struct sockaddr *) &addr, addr_len) != 0) {
		if (errno == ENOENT)
			fprintf(stderr, "Failed to connect to walnutd: walnutd " \
...
static int send_command(int sd, struct walnut_cmd_arg *cmd)
{
	unsigned long rev = WALNUT_CMD_REV;
...
/* Prints out the list of walnuts. Note: This modifies msg */
static void print_walnut_list(char *msg)
{
	char *walnut, *walnut_str, *walnut_info;
	char *saveptr1, *saveptr2;
	int field;
	/* Break msg up into walnuts */
	for (;;msg = NULL) {
		walnut = strtok_r(msg, "\30", &saveptr1);
		if (walnut == NULL)
			break;
		/* Break walnut up into info fields */
		for (field = 0, walnut_str = walnut; ; ++field, walnut_str = NULL) {
			walnut_info = strtok_r(walnut_str, "\31", &saveptr2);
			if (walnut_info == NULL)
				break;
			if (field == 0)
				printf("%s\t", walnut_info);
			else
				printf("(%s) ", walnut_info);
		}
		printf("\n");
...
static int print_response(int sd, enum walnut_cmd cmd)
{
	char *code;
	char *rmsg;
	FILE *out;
	...
	if (cmd == WALNUT_LIST && *code == '0') {
		print_walnut_list(rmsg);
		goto out;
	}
...
static int do_console(int sd, int rcv_cmd)
{
	...
	ret = open_walnut_console(fd, cmd, args);
	if (ret < 0)
		fprintf(stderr, "Failed to open console\n");
	close(fd);
...
\n\
    create <name> [--id=<id>]     Create a new walnut (optional telephony id)\n\
    destroy <name> [name ...]     Completely removes a walnut (deletes FS)\n\
    next/prev                     Select next/prev walnut as foreground\n\
    list [--all|--running|--zombie]\n\
                                  List walnuts (default: --all)\n\
\n\
    start <name> [options]        Start a walnut (see start -h for options)\n\
    stop <name>                   Stop a walnut\n\
    switch <name>                 Switch to a walnut\n\
    console <name>                Open a walnut's console\n\
\n\
    autostart <name> [on|off]     Set/Get a walnut's autostart configuration\n\
    autoswitch <name> [on|off]    Set/Get a walnut's autoswitch configuration\n\
    getid <name>                  Print a walnut's telephony ID\n\
    setid <name> <id>             Set a walnut's telephony ID\n\
    getactive                     Print the name of the active walnut\n\
    mount <name> [--all]          Mount a walnut's filesystem\n\
    unmount <name>                Unmount a walnut's filesystem\n\
    runcmd <name> <cmd> [args...] Run the command <cmd> in walnut <name>\n";

	fprintf(stderr, "Usage: %s [options]\n", name);
	fprintf(stderr, "%s", usage);
}

static int parse_help(int argc, char **argv, struct walnut_cmd_arg *cmd)
{
	int c;
...
static int parse_none(int argc, char **argv, struct walnut_cmd_arg *cmd)
{
	return parse_help(argc, argv, cmd);
}

static int get_walnutname(char *input, struct walnut_cmd_arg *cmd)
{
	...
	strcpy(cmd->walnutname, input);
	return 0;
}

static int parse_walnutname(int argc, char **argv, struct walnut_cmd_arg *cmd)
{
	int ret;
	...
	if (get_walnutname(argv[2], cmd))
		return -1;

	return 0;
}

static int parse_destroy(int argc, char **argv, struct walnut_cmd_arg *cmd)
{
	int ret;
	int i;
	...
	for (i = optind + 1; i < argc; i++) {
		ret = get_walnutname(argv[i], cmd);
		if (ret < 0)
			return ret;

		printf("Destroying walnut: %s...\n", cmd->walnutname);
		do_command(cmd);
	}
...
static int parse_create(int argc, char **argv, struct walnut_cmd_arg *cmd)
{
	int id = -1;
	int c;
	static struct option opts[] = {
	...
	if (get_walnutname(argv[optind + 1], cmd))
		return -1;
...
static int parse_list(int argc, char **argv, struct walnut_cmd_arg *cmd)
{
	struct walnut_list_args *args = &cmd->list_args;
	int id = -1;
	int c;
	static struct option opts[] = {
...
static char *dflt_argv[5] = {
	"walnut", "start", "NAME", DEFL_START_OPTS, NULL
};
...
static void print_start_help(const char *name)
{
   ...
   Valid options (default: "DEFL_START_OPTS"):\n\
    -s|--switch    make the walnut active\n\
    -w|--wait      wait for the walnut to start before returning\n\
    -C             enable walnut console\n\
    -D|--defaults  use default options (shown above)\n\
    -H             share Dalvik cache between walnuts\n\
    -M             mount unionfs (aufs) filesystem\n\
    -P <pid-file>  file in which to write global pid of init\n\
    -S             use "DEFL_SDCARD_ROOT"/<name>/sdcard as the walnut's SDCARD\n\
    -U             userid namespace\n\
...
static int parse_start(int argc, char **argv, struct walnut_cmd_arg *cmd)
{
	int c;
	struct walnut_start_args *args = &cmd->start_args;
	...
	/*
	 * The 'no' variable should be cleared when modifying the walnut startup
...
restart_default:
	...
	/* Should be 3 positional args, executable, start, and walnutname */
	if (optind != argc - 2)
		return -1;
	...
	strcpy(cmd->walnutname, argv[optind + 1]);

	return 0;
}

static int parse_autostart(int argc, char **argv, struct walnut_cmd_arg *cmd)
{
	if (parse_help(argc, argv, cmd) < 0)
		return -1;
...
static int parse_setid(int argc, char **argv, struct walnut_cmd_arg *cmd)
{
	int id;
...
static int parse_mount(int argc, char **argv, struct walnut_cmd_arg *cmd)
{
	struct walnut_mount_args *args = &cmd->mount_args;
	int id = -1;
	int c;
	static struct option opts[] = {
		{ "all",	no_argument,		NULL, 1 },
	...
	/* Should be 3 positional args, executable, mount, and walnutname */
	if (optind != argc - 2)
		return -1;
...
static int parse_runcmd(int argc, char **argv, struct walnut_cmd_arg *cmd)
{
	struct walnut_runcmd_args *args = &cmd->runcmd_args;
	char *pos;
	int i;

	/*
	 * Should be at least 4 positional args:
	 *     executable, runcmd, walnutname, command
	 */
	if (argc < 4)
		return -1;
...
struct cmd_parse {
	enum walnut_cmd cmd;
	char *cmd_name;
	int (*func)(int argc, char **argv, struct walnut_cmd_arg *cmd);
};

static struct cmd_parse cmd_names[] = {
	{ WALNUT_CREATE,		"create", 	parse_create },
	{ WALNUT_DESTROY,		"destroy", 	parse_destroy },
	{ WALNUT_LIST,		"list", 	parse_list },
	{ WALNUT_NEXT,		"next", 	parse_none },
	{ WALNUT_PREV,		"prev", 	parse_none },
	{ WALNUT_START,		"start", 	parse_start },
	{ WALNUT_STOP,		"stop", 	parse_walnutname },
	{ WALNUT_SWITCH,		"switch", 	parse_walnutname },
	{ WALNUT_CONSOLE,		"console", 	parse_walnutname },
	{ WALNUT_AUTOSTART,	"autostart", 	parse_autostart },
	{ WALNUT_AUTOSWITCH,	"autoswitch", 	parse_autostart },
	{ WALNUT_GETID,		"getid", 	parse_walnutname },
	{ WALNUT_SETID,		"setid", 	parse_setid },
	{ WALNUT_GETACTIVE,	"getactive", 	parse_none },
	{ WALNUT_MOUNT,		"mount", 	parse_mount },
	{ WALNUT_UNMOUNT,		"unmount", 	parse_walnutname },
	{ WALNUT_RUNCMD,		"runcmd",	parse_runcmd },
};
