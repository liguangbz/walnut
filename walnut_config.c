...
extern char *g_walnut_dir;

static char *get_config_path(char *name)
{
	char *config_path = malloc(strlen(g_walnut_dir) + strlen(name) +
			    strlen(CONFIG_DIR) + 8); /* +8 for '//.conf\0' */
	if (config_path == NULL) {
		ALOGE("Failed to malloc for config_path: %s", strerror(errno));
		return NULL;
	}
	sprintf(config_path, "%s/%s/%s.conf", g_walnut_dir, CONFIG_DIR, name);
	return config_path;
}

static char *get_config_dir()
{
	char *config_dir = malloc(strlen(g_walnut_dir) + strlen(CONFIG_DIR) + 2);
	if (config_dir == NULL) {
		ALOGE("Malloc of config_dir failed: %s", strerror(errno));
		return NULL;
	}
	sprintf(config_dir, "%s/%s", g_walnut_dir, CONFIG_DIR);
	return config_dir;
}
...
/* Check's if a given ID is already being used by a walnut.
...
int id_exists(int id)
{
	...
	char **name_list = get_walnut_names();
	if (name_list == NULL)
		return -1;
...
struct conf_key_offset config_keys[] = {
	{ "newwalnut",	offsetof(struct config_info, newwalnut) },
...
/* Used to free the list of walnuts obtained from get_walnut_names() */
void free_walnut_names(char **name_list)
{
	int i;
	for (i = 0; name_list[i] != NULL; ++i) {
		free(name_list[i]);
...
/* Returns a list of walnuts with config files AKA a list of walnuts that exist */
char **get_walnut_names(void)
{
	int ret, i = 0;
	DIR *dir;
	struct dirent entry;
...
err_get_walnut_names:
	ALOGE("malloc/realloc during get_walnut_names failed");
	closedir(dir);
	name_list[i] = NULL;
	free_walnut_names(name_list);
	return NULL;
...
void start_args_to_config(struct walnut_start_args *args,
			  struct config_info *config)
{
	 ...
	 * the 'autoswitch' walnutd command instead
	 */
}
